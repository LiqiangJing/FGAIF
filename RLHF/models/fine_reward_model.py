...
class RewardModel(transformers.PreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert self.num_labels != None, "num_labels is None!"
        reward_head = nn.Linear(hidden_size, self.num_labels)
    ...
    def forward(
        self, input_ids, labels, attention_mask=None, images=None, return_dict=True, **kwargs
    ):
        ...
        # exit()
        return RewardModelOutput(rewards=rewards, new_labels=labels) if return_dict else (rewards,)
...
class RewardModelTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        input_ids, attention_mask, images, labels = unpack_dict(
            inputs,
            keys=(
                "input_ids",
                "attention_mask",
        ...
        # repeat images to match the number of candidates images (batch, h, w, c)
        ...
        # )
        input_ids_flat, attention_mask_flat = input_ids, attention_mask
        outputs = model(
            input_ids=input_ids_flat, labels=labels, attention_mask=attention_mask_flat, images=images
        )
        rewards = outputs.rewards   ## (b, len, logits)
        # rewards = einops.rearrange(
        #     rewards_flat, "(b c) -> b c", c=num_candidates
        # )  # Size: (bsz, num_candidates).
        # print(rewards)
        # print()
        # print(outputs['new_labels'])
        # # print()
        # print("rewards", rewards.view(-1, model.num_labels).shape)
        # print("outputs", outputs['new_labels'].shape) ## 3396 (4, 849)
        # exit()
        # print(outputs['new_labels'].shape)
        loss = F.cross_entropy(rewards.view(-1, 3), outputs['new_labels'].view(-1), reduction="mean", ignore_index=-100)
        # print(loss)
...
def compute_reward_modeling_metrics(eval_prediction: EvalPrediction) -> Dict:
    ...
    # print(type(eval_prediction.label_ids)) #np
    # print(eval_prediction.label_ids)
    # print(eval_prediction.label_ids.shape)
    # print(type(eval_prediction.predictions)) #tuple
    # print(eval_prediction.predictions)
    # print(eval_prediction.predictions[0].shape) #(5, 849, 32000)
    # print(eval_prediction.predictions[1].shape) #(5, 849)

    labels = eval_prediction.predictions[1]
    # print(labels)
    predictions = np.argmax(eval_prediction.predictions[0], axis=2)
    # print(eval_prediction.predictions[0])
    # print(eval_prediction.predictions[1])
    all_num = 0
    correct = 0
    ...
    # average_score = torch.tensor(predictions).float().mean().item()
    return dict(
