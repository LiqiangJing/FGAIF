...
class AutoregressiveValue(Value):
    def forward(
        ...
        if self.args.init_value_with_reward:
            inputs = self.base_model.prepare_inputs_for_generation(
                input_ids=sequences,
        ...
        # values = values * sequence_attn_masks + 0 * (1 -query_attn_masks)
...
class ActorCritic(nn.Module):
    def __init__(self, policy: Policy, value_model: Value):
        ...
        #         if "lora_policy" not in name:
        #             print(name)
        # print("value")
        # for name, param in value_model.named_parameters():
        #     if param.requires_grad:
        #         if "lora_value" not in name:
        #             print(name)
        # exit()
                ...
                * torch.sum(
                    torch.stack(
                        [
                            ...
                            if "lora_value" in key or "value_head" in key
                        ]
                    )
                )
            }
                ...
                * torch.sum(
                    torch.stack(
                        [
                            torch.mean(value)
                            for key, value in self.named_parameters()
                            if "lora_policy" in key or "lora_value" in key
                        ]
                    )
                )
            }
