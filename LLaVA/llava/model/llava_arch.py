...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images)
        # print("image_features", image_features.shape) # 8, 576, 5120
        # print("image shape", input_ids.shape) 8， 665
        # print("label shape", labels.shape) None
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                # print("cur_input_embeds", cur_input_ids.shape)
                new_input_embeds.append(cur_input_embeds)
                if labels is not None:
                    new_labels.append(labels[batch_idx])
                cur_image_idx += 1
            ...
            if labels is not None:
                ...
                cur_new_labels = []
                assert cur_labels.shape == cur_input_ids.shape
            while image_token_indices.numel() > 0: ## number of images
                cur_image_features = image_features[cur_image_idx]
                image_token_start = image_token_indices[0]
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    self.config, "mm_use_im_start_end", False
                    ...
                    # Run through this
                    # print("image_features.shape", image_features.shape) #[8, 576, 5120]
                    # print(self.get_model().embed_tokens(cur_input_ids[:image_token_start]).shape) #[35, 5120])
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(cur_input_ids[:image_token_start])
                    )
                    # print("1", self.get_model().embed_tokens(cur_input_ids[:image_token_start]).shape)
                    # print("2", cur_image_features.shape)
                    cur_new_input_embeds.append(cur_image_features)
                    if labels is not None:
                        cur_new_labels.append(cur_labels[:image_token_start])
                        cur_new_labels.append(
                ...
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    ...
                    ## run throght this block
                    cur_input_ids = cur_input_ids[image_token_start + 1 :]
                image_token_indices = torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0]

            if cur_input_ids.numel() > 0: ## return the number of the elements in the list
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    self.config, "mm_use_im_start_end", False
                ):
                    cur_new_input_embeds.append(
                    ...
                    # print("3",  self.get_model().embed_tokens(cur_input_ids).shape)
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(cur_input_ids)
                    )
                if labels is not None:
            ...
            cur_new_input_embeds = torch.cat(cur_new_input_embeds, dim=0)
            # print("4", cur_new_input_embeds.shape)
            new_input_embeds.append(cur_new_input_embeds)
            if labels is not None:
                cur_new_labels = torch.cat(cur_new_labels, dim=0)
                new_labels.append(cur_new_labels)
        # print(any(x.shape != new_input_embeds[0].shape for x in new_input_embeds)) False
        # print(attention_mask.shape) 8,765
        # exit()
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            max_len = max(x.shape[0] for x in new_input_embeds)
            new_input_embeds_align = []
            ...
            new_input_embeds = torch.stack(new_input_embeds, dim=0)
            # for item in new_input_embeds:
            #     print("sss", item.shape)
            # print(new_input_embeds.shape) #[8, 1240, 5120]
            # exit()
            if labels is not None:
                new_labels = torch.stack(new_labels, dim=0)
    ...
    def prepare_inputs_labels_for_multimodal_fine(
            self, input_ids, attention_mask, past_key_values, labels, images
    ):
        vision_tower = self.get_vision_tower()
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images)
        # print("image shape", image_features.shape) # 8, 576, 5120
        # print("image shape", input_ids.shape) 8， 665
        # print("label shape", labels.shape) None
